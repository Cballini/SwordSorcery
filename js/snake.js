...
function resetGame(){
	...
	{
 	   $(this).css('background-image', "url('img/snake/backgroundGame.png')");
	}).fadeTo('slow', 1);
...
function creer_map(){
	...
	$("div#"+section).append("<table class='endGame'>"+tr.join("\n")+"</table>");
}
...
function act_snake()
{
	...
	{	
		gameOver("Vous êtes bloqués. Attention nombre de vies :" + getLife());
		return true;
	}
	else if(nvl_tete[0] < 0 || nvl_tete[1] < 0)
	{
		gameOver("Hors limite. Attention nombre de vies :" + getLife());
		return true;
	}
...
function gameOver(message)
{
	...
	$('table').fadeTo('slow', 0.5, function()
	{
 	   $(this).css('background-image', "url('img/snake/youdied.jpg')");
	}).fadeTo('slow', 1);
	...
	if(getLife() <= 0)
	{
		$("#"+section+" button:eq(3)").show();
	}
	else
	{
		$("#"+section+" button:eq(0)").show(); //reset
	}
...
function catchFruit(pos)
{
	...
	{
		gameOver("Un ennemi a attrapé une amie de Finn et Jake ! Attention nombre de vies :" + getLife());
	}
	else if(cell.hasClass("tete_snake"))
	{
		gameOver("Un ennemi a attrapé Finn ! Attention nombre de vies :" + getLife());
	}
}
